...
def main():
	...
	for city in foundFiles:
		...
		if complete:
			...
			print file
			try:
				completeCities+=[{'name':city.split('/')[1],'province':city.split('/')[0],'dirname':city}]
			except:
				print 'error'
	...
	for city in completeCities:
		print city + " " + str(completeCities[city])
	...
	newId=1
	for city in completeCities:
		cityRow = [ newId,city['name'],city['province'],city['dirname']]
